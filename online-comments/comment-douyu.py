...
def get_longinres(s_ip=b'117.79.132.20', s_port=8001, rid=b'5275'):
...
def main(url='http://www.douyutv.com/meizhi'):
...
def send_to_slack(username, text):
    if WEBHOOK_URL:
...
def _send_to_slack():
    ...
    print('2', WEBHOOK_URL)
    while True:
        username, text = queue.get()
        payload = {"channel": "#dychat", 
                "username": username, 
                "text": text}
